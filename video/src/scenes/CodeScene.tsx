...
export const CodeScene: React.FC<SceneProps> = ({visual, sentences}) => {
  ...
  if (longestLine > 110) fontSize = Math.min(fontSize, 18);
  else if (longestLine > 105) fontSize = Math.min(fontSize, 20);
  else if (longestLine > 94) fontSize = Math.min(fontSize, 22);
  else if (longestLine > 84) fontSize = Math.min(fontSize, 24);
